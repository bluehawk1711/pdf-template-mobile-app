...
export const Page7: React.FC = () => {
  ...
  const listOp = mk(); const listY = mk(10);
  const prescOp = mk();
  const imgOp = mk(); const imgSc = mk(0.7);
  const ingOps = useRef(INGREDIENTS.map(() => new Animated.Value(0))).current;
  const tagOp = mk(); const tagY = mk(10);
  const alsoOp = mk();

  useEffect(() => {
    Animated.sequence([
      Animated.parallel([
        Animated.timing(listOp, { toValue: 1, duration: 400, delay: 50, useNativeDriver: true }),
        Animated.timing(listY, { toValue: 0, duration: 400, delay: 50, useNativeDriver: true }),
        Animated.timing(prescOp, { toValue: 1, duration: 400, delay: 150, useNativeDriver: true }),
        Animated.timing(imgOp, { toValue: 1, duration: 500, delay: 100, useNativeDriver: true }),
        Animated.timing(imgSc, { toValue: 1, duration: 500, delay: 100, useNativeDriver: true }),
      ]),
      Animated.parallel([
        ...ingOps.map((op, i) =>
          Animated.timing(op, { toValue: 1, duration: 400, delay: i * 80, useNativeDriver: true }),
        ),
      ]),
      Animated.parallel([
        ...
        Animated.timing(alsoOp, { toValue: 1, duration: 400, delay: 100, useNativeDriver: true }),
      ]),
    ]).start();
  }, [listOp, listY, prescOp, imgOp, imgSc, ingOps, tagOp, tagY, alsoOp]);

  /* ── font sizes ──────────────────────────────────────────────────── */
  const fListT = bW * 0.020;
  ...
  const fTag = bW * 0.032;
  const fAlsoT = bW * 0.020;
  const fAlsoB = bW * 0.030;
  ...
  return (
    <View style={styles.screen}>
      ...
      <Animated.View style={{
        position: 'absolute', left: bL + bW * 0.02, top: bT + bH * 0.50,
        width: bW * 0.28, opacity: listOp, transform: [{ translateY: listY }],
      }}>
        <Text style={{
          fontFamily: SANS, fontSize: fListT, fontWeight: '700',
        ...
        {PAIN_LIST.map((item, i) => (
          <Text key={i} style={{
            ...
            lineHeight: fList * 1.3, marginBottom: bH * 0.001,
          }}>
            {'- '}{item}
          </Text>
        ))}
      ...
      <Animated.View style={{
        position: 'absolute', left: bL + bW * 0.02, top: bT + bH * 0.68,
        width: bW * 0.28, opacity: prescOp,
      }}>
        <Text style={{
          fontFamily: SANS, fontSize: fPrescT, fontWeight: '700', color: HEADING,
        }}>
          Please Prescribe :
        </Text>
        <Text style={{
          fontFamily: SERIF, fontSize: fPrescB, fontWeight: '800', fontStyle: 'italic',
          color: BLUE, marginVertical: bH * 0.002,
        }}>
          Qutoflam-SP
        </Text>
      ...
      {INGREDIENTS.map((ing, i) => {
        const sectionTop = bT + bH * (0.50 + i * 0.11);
        return (
          <Animated.View key={ing.title} style={{
            ...
            <Text style={{
              fontFamily: SANS, fontSize: fIngH, fontWeight: '700',
              color: HEADING, marginBottom: bH * 0.002,
            }}>
              {ing.title}
            </Text>
            {ing.lines.map((line, j) => (
              <Text key={j} style={{
                ...
                lineHeight: fIngX * 1.3, marginBottom: bH * 0.001,
              }}>
                {'◆ '}{line}
              </Text>
            ))}
      ...
      <Animated.View style={{
        ...
        <Text style={{
          fontFamily: SANS, fontSize: fAlsoX, fontWeight: '600', color: LT_GRAY,
        }}>
